...
//
// Everything runs inside one top-level try/catch that always returns
// Response.json — see trigger-workflow-run/route.ts header for why this
// matters (Hasura requires valid JSON on every path; an unhandled
// exception previously produced "not a valid json response from webhook").
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("approve-step: unhandled error:", err);
