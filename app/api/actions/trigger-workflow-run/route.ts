...
//
// IMPORTANT: Hasura Actions require a valid JSON response on every
// possible path. An unhandled exception anywhere in this handler
// previously produced a non-JSON error page, which Hasura reports as
// "not a valid json response from webhook" — a real failure mode hit in
// production when NHOST_GRAPHQL_URL/NHOST_ADMIN_SECRET were misconfigured.
// Everything below now runs inside one top-level try/catch that always
// returns Response.json, even for totally unexpected errors.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Top-level safety net: guarantees valid JSON even for totally
    // unexpected errors (e.g. missing env vars, Hasura connectivity
    // failures). Logs the real error server-side (visible in `vercel
    // logs`) while still returning a clean, informative JSON body to
    // Hasura instead of crashing into a non-JSON error page.
    const message = err instanceof Error ? err.message : String(err);
    console.error("trigger-workflow-run: unhandled error:", err);
